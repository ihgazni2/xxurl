...
class URL():
    ...
    def repl_protocol(self,new_scheme,**kwargs):
        urlstr = replace_protocol(self.urlstr,new_scheme,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.origin = get_origin(urlstr)
    ...
    def repl_netloc(self,new_netloc,**kwargs):
        urlstr = replace_netloc(self.urlstr,new_netloc,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.origin = get_origin(urlstr)
        ...
        nlocd = eded._select_norecur(nind,'username','password','hostname','port')
        self.netloc = packup_netloc(nlocd)
    def repl_userinfo(url,new_userinfo,**kwargs):
        urlstr = replace_userinfo(self.urlstr,new_userinfo,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.origin = get_origin(urlstr)
    ...
    def repl_username(self,new_username,**kwargs):
        urlstr = replace_username(self.urlstr,new_username,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.origin = get_origin(urlstr)
    ...
    def repl_password(self,new_password,**kwargs):
        urlstr = replace_password(self.urlstr,new_password,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.origin = get_origin(urlstr)
    ...
    def repl_host(self,new_host,**kwargs):
        urlstr = replace_host(self.urlstr,new_host,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.origin = get_origin(urlstr)
    ...
    def repl_hostname(self,new_hostname,**kwargs):
        urlstr = replace_hostname(self.urlstr,new_hostname,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.origin = get_origin(urlstr)
    ...
    def repl_port(self,new_port,**kwargs):
        urlstr = replace_port(self.urlstr,new_port,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.origin = get_origin(urlstr)
        ...
        nlocd = eded._select_norecur(nind,'username','password','hostname','port')
        self.netloc = packup_netloc(nlocd)
    def repl_path(url,new_path,**kwargs):
        urlstr = replace_path(self.urlstr,new_path,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.path = nind['path']
        self.pathname = nind['path']
    def repl_params(url,new_params,**kwargs):
        urlstr = replace_params(self.urlstr,new_path,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.params = nind['params']
    def repl_query(url,new_query,**kwargs):
        urlstr = replace_params(self.urlstr,new_path,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.query = nind['query']
        self.search = nind['search']
    def repl_fragment(url,new_fragment,**kwargs):
        urlstr = replace_fragment(self.urlstr,new_fragment,**kwargs)
        nind = nin_u2d(urlstr)
        self._nind = nind
        self.href = urlstr
        self.fragment = nind['fragment']
